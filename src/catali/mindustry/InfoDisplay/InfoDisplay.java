...
import catali.mindustry.InfoDisplay.types.TextInputPack;
import catali.mindustry.InfoDisplay.types.TimedPack;
import catali.mindustry.world.MapControl;

import static catali.NekoVars.taskScheduler;

public class InfoDisplay {
    private static final InfoDisplay infoDisplay = new InfoDisplay();
    ...
    private void displayContent(Player player, DisplayPack displayPack, boolean isFollowUp) {
        try {
            if (isFollowUp) {
                Call.followUpMenu(player.con, displayPack.id, displayPack.title, displayPack.display,
                        displayPack.options);
            } else {
                Call.menu(player.con, displayPack.id, displayPack.title, displayPack.display, displayPack.options);
            }
            HashMap<String, TimedPack<DisplayPack>> map = displayPacks.computeIfAbsent(displayPack.id,
                    k -> new HashMap<>());
    ...
    public void showInput(Player player, TextInputPack textInputPack) {
        if (mapControl.isGame())
            try {
                Call.textInput(player.con, textInputPack.id, textInputPack.title, textInputPack.message,
                        MAX_MESSAGE_LENGTH,
                        "", textInputPack.isNumeric);
                HashMap<String, TimedPack<TextInputPack>> map = textInputPacks.computeIfAbsent(textInputPack.id,
                        k -> new HashMap<>());
    ...
    private void cleanUpExpiredEntries() {
        ...
        displayPacks.values()
                .forEach(map -> map.values().removeIf(timedPack -> now - timedPack.timestamp > EXPIRATION_TIME));
        textInputPacks.values()
                .forEach(map -> map.values().removeIf(timedPack -> now - timedPack.timestamp > EXPIRATION_TIME));
    }
    ...
    public void onMenuOptionChoose(MenuOptionChooseEvent event) {
        ...
        if (event.option == -1 || timedDisplayPack == null)
            return;
    ...
    public void onTextInput(TextInputEvent event) {
        ...
        if (event.text == null || timedTextInputPack == null)
            return;
